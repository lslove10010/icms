 ...
 */
function searchAPI(data, keyword) {
  let searchArr = [];
  let searchAPIDom = "";
  let data1 = {};
  data.forEach((item) => {
  ...
  if (Object.keys(data1).length > 0) {
    ...
    Object.keys(data1).forEach((key, value) => {
      ...
      <li class="nav-item">
          <a href="javascript:void(0);" class="nav-link nav-link1 " style="font-size: 16px;">
             ${data1[key].Name}
           </a>
       </li>
      ${str[key]}</ul>`;
...
// 搜索后续处理
function getSearchDom(data, keyWord) {
  if (keyWord) {
    let abc = searchAPI(data, keyWord);
    $(".pcMenu").hide();
    $(".searchMenu").html(abc);
    $(".searchMenu").show();
  } else {
...
function getSearchH5Dom(data, keyWord) {
  if (keyWord) {
    let abc = searchAPI(data, keyWord);
    $(".pcMenu").hide();
    $(".searchMenu").html(abc);
